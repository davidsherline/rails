...
Active Record supports automatic identification for most associations with standard names. However, Active Record will not automatically identify bi-directional associations that contain the following options:
...
Active Record provides the `:inverse_of` option so that you can declare bi-directional associations that it cannot recognize automatically:
...
By including the `:inverse_of` option in the `has_many` declaration, Active Record will now recognize the bi-directional association:
